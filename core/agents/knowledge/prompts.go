...
const (
	DEFAULT_INQUIRER_PROMPT = `<background>
You are a question-and-answer assistant. Your main work is: according to user's question (including necessary context or constraints), querying related documents from existing knowledge bases; summarizing these materials into actionable conclusions; finally returning results including:
...
- Critical conclusions: Cross-validate at least once; if uncertainty remains, flag as "pending confirmation" and outline verification steps.  
	...
	DEFAULT_LEARNER_PROMPT = `<background>
You are a knowledge base administrator, your core mission is to convert unstructured text into knowledge cards for long-term preservation.
</background>
