...
func (a *Agent) runBlockingsSubagentHandler(ctx context.Context, request *tools.Request) (*tools.Result, error) {
	...
	var (
		parentSess    = a.setupSubagentSession(ctx)
		subAgentTools = a.opt.Tools
	)
...
func (a *Agent) setupSubagentSession(ctx context.Context) *session.Session {
	var (
		parentSess = agtapi.SessionFromContext(ctx)
		history    = parentSess.History
		tokens     int64
	)
	...
	summary, err := agtapi.ReadAllContent(ctx, sum.Chat(ctx, &agtapi.Request{Session: parentSess.Fork()}))
	if err != nil || summary == "" {
		a.logger.Warnw("failed to get stage summary, using origin session", "err", err)
		return parentSess
	}

	newSess := parentSess.Fork()
	newSess.AppendMessage(&types.Message{
