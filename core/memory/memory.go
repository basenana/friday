...
func (m *Memory) Scratchpad() tools2.Scratchpad {
...
func (m *Memory) Tools() []*tools2.Tool {
	if m.tools != nil {
		return m.tools
	}

	memoryTools := make([]*tools2.Tool, 0)
	if v := m.session.Scratchpad(); v != nil {
		memoryTools = append(memoryTools, tools2.ScratchpadReadTools(v)...)
	}
	m.tools = memoryTools
	return m.tools
}
