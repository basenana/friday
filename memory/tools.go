...
func ListStorageTools() []*tools.Tool {
	return []*tools.Tool{
		...
		tools.NewTool("retrieve_from_file",
			tools.WithDescription("The absolute path to the local file. If you don't know the absolute path, you need to use `list_persistence_files` to find it."),
			tools.WithString("path",
				tools.Required(),
				tools.Description("This tool reads locally saved files; it does not support any write operations or network requests."),
			),
		...
		tools.NewTool("write_content_to_file",
			...
			tools.WithString("path",
				tools.Required(),
				tools.Description("The absolute path to the file that needs to be saved must be provided. This file will be accessed by multiple agents, so please ensure the path remains readable."),
			),
