...
func ListStorageTools() []*tools.Tool {
	return []*tools.Tool{
		tools.NewTool("list_persistence_files",
			tools.WithDescription("View all files in the current records"),
			tools.WithToolHandler(handleListPersistenceFileTools),
		),
		tools.NewTool("retrieve_from_file",
			tools.WithDescription("read file content"),
			tools.WithString("path",
				tools.Required(),
				tools.Description("the abs path of file"),
			),
			tools.WithToolHandler(handleRetrieveFromFileTools),
		),
		tools.NewTool("write_content_to_file",
			tools.WithDescription("Store the reused data in a file so that subsequent questions can be quickly retrieved."),
			tools.WithString("path",
				tools.Required(),
				tools.Description("the abs path of file"),
			),
