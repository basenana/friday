...
Friday is an intelligent QA system using RAG (Retrieval-Augmented Generation) that matches user queries against a vectorized knowledge base and summarizes responses via LLMs (OpenAI, Gemini, GLM-6B). It includes a multi-agent framework with tool-calling capabilities.
...
### Core Packages (`pkg/`)
- `friday/` - Core QA logic (question, ingest, summary, keywords)
- `llm/` - LLM clients for OpenAI, Gemini, GLM-6B
- `embedding/` - Vectorization (OpenAI, Gemini, HuggingFace)
- `store/` - Data layer (Postgres, Redis, MeiliSearch, PGVector)
- `tools/` - Tool definitions with JSON schema support
- `actor/` - Actor pattern for concurrent agents
- `dispatch/` - Thread pool dispatching
