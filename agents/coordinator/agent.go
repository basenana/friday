...
func prebuildMailTool(agt *Agent) []*tools.Tool {
	return []*tools.Tool{
		tools.NewTool("send_text_mail",
			tools.WithDescription("向其他 Agent 发送文本邮件，用于其他 Agent 请求帮助"),
			tools.WithString("to_agent_names",
				tools.Required(),
				tools.Description("接受者的 Agent 名称，如果有多个用英文,分割"),
			),
			tools.WithString("title",
				tools.Required(),
				tools.Description("简单总结请求的问题"),
			),
			tools.WithString("text",
				tools.Required(),
				tools.Description("精简的描述需要请求解决的问题，建议排查思路，和期望获得的回复"),
			),
